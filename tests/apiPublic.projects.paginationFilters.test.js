...
// Mock Projects before requiring app.js
jest.mock('../models/Projects.js', () => ({
  find: mockFind,
}));
...
describe('GET /api/projects pagination & filters', () => {
  ...
  it('applies pagination with skip and limit', async () => {
    ...
    const res = await agent.get('/api/projects?page=3&perPage=5');
  ...
  it('adds industry filter when provided (single)', async () => {
    const filterCapture = [];
    mockFind.mockImplementation((filter) => { filterCapture.push(filter); return buildFindChain({ result: [] }); });

    const res = await agent.get('/api/projects?industry=abc123');

    expect(res.status).toBe(200);
    expect(filterCapture[0]).toEqual({ isPubliclyVisible: true, industries: { $in: ['abc123'] } });
  });

  it('adds services filter when provided (multiple)', async () => {
    const filterCapture = [];
    mockFind.mockImplementation((filter) => { filterCapture.push(filter); return buildFindChain({ result: [] }); });

    const res = await agent.get('/api/projects?services=a&services=b');

    expect(res.status).toBe(200);
    expect(filterCapture[0]).toEqual({ isPubliclyVisible: true, serviceTypes: { $in: ['a', 'b'] } });
  });
  ...
  it('adds featured flag when featured=true', async () => {
    ...
    const res = await agent.get('/api/projects?featured=true');
